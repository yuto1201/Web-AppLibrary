# プロダクト仕様

AppLibrary は個人開発したアプリの紹介サイト。訪問者が一覧から各アプリの特徴、スクリーンショット、配布先、プライバシーポリシーを確認できる。

- 掲載対象は iOS / iPadOS / macOS / watchOS / visionOS / Web / CLI。
- アプリ情報は `src/data/registry.ts`、本文は `src/data/privacy/`、画像は `public/apps/` に保持する。
- 表示設定はテーマと言語のみ。端末の localStorage に保存し、アカウントを作らない。
- Next.js App Router の静的出力を Vercel からドメインルートへ配信する。Cloudflare は DNS only。
- トップページは紙 (paper) の意匠。掲載数が一覧の絞り込みを必要としないため、検索とフィルタは持たない。掲載アプリが 10 件を超えたら再検討する。
- 遊びはステッカー帯 1 箇所に集約する。アプリアイコンを掴んで動かせる。
- 公開 URL と掲載内容は意匠の変更で変えない。

DB、認証、API サーバー、Supabase、Cursor Cloud の起動、テンプレート生成、プロバイダー操作の自動化は本移植の対象外。将来必要になった時は別 Issue として設計する。

サイト共通の利用規約は `/terms/`、ウェブサイトのプライバシーポリシーは `/privacy/` で静的公開する。アプリ別のプライバシー本文はテンプレートの未確認文言を公開せず、現行実装・既存公開文書・公開 App Store のプライバシー表示と照合する。アプリ固有の利用規約が必要な場合は、ユーザー承認済み本文だけを `/apps/<slug>/terms/` へ静的公開する。

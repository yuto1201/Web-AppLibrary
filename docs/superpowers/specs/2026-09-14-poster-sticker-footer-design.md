...
| `--ink` | `#000000` | 本文 |
| `--rule` | `#e2e8f0` | 罫線 |
| `--accent` | `#006eff` | 見出し・ナビ・outlined 操作。本文には使わない |

ダークは同じ役割の反転（濃いインク紙）。本文は明るいインク、アクセントは電圧ブルーのまま。16px 文字で 4.5:1 を割ったら **dark 専用の明るいブルー** を `--accent` にだけ足す。紙面に影・グラデを足さない。
...
個別ページと法務はトークンと Nav を追随させる。シール山はトップだけ。
...
- [ ] light のキャンバスが `#fff8f1`、本文が黒、サイトの操作色が `#006eff`。紙面にドロップシャドウがない。
...
- [ ] 個別アプリページと法務が新しい紙面トークンで破綻しない。
- [ ] `npm run verify` が通る。

### Issue B — シール山とページ全体ドラッグ

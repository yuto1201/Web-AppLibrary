...
書体は 4 つ。`--font-sans` (Inter 300/400) が本文と UI、`--font-display` (Newsreader) が Latin のワードマークと英語見出し、`--font-mono` (JetBrains Mono) が日付と件数、`--font-hand` (Klee One 400) が手書き合図と鉛筆メモだけ。Klee One は `src/fonts/KleeOne-Regular.ttf` を `next/font/local` で自己ホストする（`next/font/google` の latin subset では日本語が落ちる）。本文・ナビ・CTA・ワードマークには使わない。display は latin subset なので、日本語の本文は OS のゴシックへ落ちる。Bricolage Grotesque は使わない。
...
3. `AppsSection`: 掲載アプリを**行の索引**として並べる。行全体が個別ページへのリンク。hover の色はアプリ自身の `accent` を使う。ステッカーと `slug` で相互にハイライトする。見出しの下に CSS のマスキングテープ（`.desk-tape`）が少し回転して覗く。見出し文字の背面には置かず、コントラストを落とさない。
4. `Stickers`: 既存アイコンを白フチのビニールとして、**最初の画面から紙に乗せる**。配置カタログは `src/lib/sticker-desk.ts` の `DESK_ITEMS`。読み込みごとに乱さない。
...
- 移動量がしきい値未満なら「タップ」として個別ページへ遷移する。各アプリステッカーは `<Link>` なのでキーボードでは通常のリンクとして動く。

...
掴んでいる間、元の位置に `.sticker-ghost`（1px 破線）を重ねる。ステッカー本体は `transform` で動くが、`.sticker-slot`（フッター山の初期位置に留まる）へ `inset:0` で重ねているため、常に元の位置と正確に一致する。`.sticker-stage` は `.poster` 全面を覆うが `pointer-events: none` で、ヒットはシール自身だけが受ける。
